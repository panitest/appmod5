...
function alargar(){
	...
	if(! document.getElementById('overlay')){
	    ...
	    try{set_opacity(ov, .9,'panino5001');}catch(e){}
		document.getElementsByTagName('body')[0].appendChild(ov);
		set_opacity(_('overlay'), .9,'panino5001');
	}
...
function ventana(W,H,T){
	if( !_('qq'))addIfr();
	_('qq').innerHTML='';
	_('qq').style.width=W+'px';
	_('qq').style.height=H+'px';
	_('qq').style.backgroundImage='url(images/ajax-loader.gif)';
	window.popup=true;
	var pagina=getWindowData();
	if(!T){
		_('qq').style.top=(pagina[1]/2)-(H/2)+pagina[3]+'px';
	}else{
		_('qq').style.top=T+'px';
	}
	_('qq').style.left=(pagina[0]/2)-(W/2)+pagina[2]+'px';
	_('qq').style.zIndex='10000';
	alargar();
...
function cerrar(){
	...
	_('qq').innerHTML='';
	_('qq').style.top='-1500px';
	_('qq').style.left='-1500px';
	_('qq').style.zIndex='10';
	if(document.getElementById('overlay'))
...
function addIfr(){
		if(!_('qq')){
			var cad='<div style="position:absolute;left:-1500px;top:-1500px; background:url(images/squares.svg) 50% 50% no-repeat; text-align:center;z-index:120" id="qq"></div>';
